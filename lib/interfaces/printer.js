...
class Printer extends Interface {
  ...
  getPrinterName() {
    let { name } = this;
    if (!name || name === "auto") {
      const pl = this.driver
        .getPrinters()
        .filter((p) => p.attributes.indexOf("RAW-ONLY") > -1);
      if (pl.length > 0) {
        name = pl[0].name;
      }
    }
    if (!name || name === "auto") {
      throw new Error(
        "A RAW-ONLY Printer could not be detected. Please configure a Printer-Name"
      );
    }
    return name;
  }
  ...
  async isPrinterConnected() {
    ...
    let foundPrinter = this.driver.getPrinter(name);
    if (!foundPrinter) {
      const printers = this.driver.getPrinters();
      foundPrinter = printers.find((p) => p.name === name);
    }
    if (
      foundPrinter &&
      foundPrinter.status.indexOf("NOT-AVAILABLE") === -1 &&
      foundPrinter.attributes.indexOf("OFFLINE") === -1
    ) {
      return true;
    }
    return false;
